// FRM01255_CommitteeMinutes.jsx
// FRM-01255 – Committee Minutes
...
const validationSchema = Yup.object({
  ...
  committeeName: Yup.string().required("Required"),
...
const initialValues = {
  ...
  attendance: [
    { memberName: "", status: "", signature: "", dynamicFields: {} }
  ],
...
const FRM01255_CommitteeMinutes = () => {

  const { isPrintMode } = usePrintMode();
  const [attendanceColumns, setAttendanceColumns] = useState([]);
  const [proceedingColumns, setProceedingColumns] = useState([]);
  ...
  const addColumn = (type) => {
    ...
    if (type === "attendance") {
      ...
      if (proceedingColumns.find(col => col.key === key)) return;
      setProceedingColumns([...proceedingColumns, { key, label: columnName }]);
    }
  };
  ...
  const removeColumn = (type, key) => {
    if (type === "attendance") {
      ...
      setProceedingColumns(proceedingColumns.filter(col => col.key !== key));
    }
  };
  ...
  return (
    <ModernFormWrapper formId="FRM-01255" title="Committee Minutes">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Committee Minutes Submitted Successfully");
        }}
      >
            ...
            >
              ...
              <div className="form-section">
                ...
                <div className="form-fields">
                  ...
                  {field(values,"committeeName","Committee Name")}
...
export default FRM01255_CommitteeMinutes;