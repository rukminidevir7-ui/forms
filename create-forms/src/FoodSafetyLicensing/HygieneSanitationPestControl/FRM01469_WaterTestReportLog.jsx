// FRM01469_WaterTestLog.jsx
// FRM-01469 – Water Test Log
...
const FRM01469_WaterTestLog = () => {
  ...
  return(
    <ModernFormWrapper formId="FRM-01469" title="Water Test Log">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Water Test Log Submitted Successfully");
        }}
      >
      {({values,setFieldValue})=>(
        <Form>
          <ModernA4Template
            formId="FRM-01469"
            title="FRM-01469 — Water Test Log"
          ...
          <div className="form-section">
            <h3 className="form-section-title">Test Log Entries</h3>
            ...
            {!isPrintMode &&
              <div style={{marginBottom:15}}>
                ...
                <button
                  ...
                  style={{marginLeft:10}}
                  onClick={() =>
                    setFieldValue("testLogEntries", [
                      ...values.testLogEntries,
                      {
                        sampleId:"",
                        samplingLocation:"",
                        testType:"",
            ...
            <FieldArray name="testLogEntries">
            {({remove})=>(
              <table className="items-table">
                ...
                <tbody>
                  {values.testLogEntries.map((row,index)=>(
                    <tr key={index}>
                      {["sampleId","samplingLocation","testType","parametersTested","resultSummary","complianceStatus","testDate","laboratory","nextTestDue","remarks"].map(field=>(
                        <td key={field}>
                          <Field
                            name={`testLogEntries.${index}.${field}`}
                            type={
                              field==="testDate" || field==="nextTestDue"
                                ? "date"
                                : "text"
                            }
                            as={["resultSummary","remarks"].includes(field) ? "textarea" : "input"}
                            className="form-input"
                          />
                        </td>
                      ))}
                      {dynamicColumns.map(col=>(
                        <td key={col.key}>
                          <Field
                            name={`testLogEntries.${index}.dynamicFields.${col.key}`}
                            className="form-input"
                          />
                        </td>
                      ))}
...
export default FRM01469_WaterTestLog;