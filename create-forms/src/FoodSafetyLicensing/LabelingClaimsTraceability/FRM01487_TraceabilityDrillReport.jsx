// FRM01487_TraceabilityDrillReport.jsx
// FRM-01487 – Traceability Drill Report
...
const initialValues = {
  ...
  /* 5. Drill Execution Details */
  drillExecutionDetails: [
    {
      stepDescription: "",
      responsible: "",
      startTime: "",
...
const FRM01487_TraceabilityDrillReport = () => {
  ...
  return(
    <ModernFormWrapper formId="FRM-01487" title="Traceability Drill Report">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Traceability Drill Report Submitted Successfully");
        }}
      >
      {({values,setFieldValue})=>(
        <Form>
          <ModernA4Template
            formId="FRM-01487"
            title="FRM-01487 — Traceability Drill Report"
          ...
          <div className="form-section">
            <h3 className="form-section-title">Drill Execution Details</h3>
            ...
            {!isPrintMode &&
              <div style={{marginBottom:15}}>
                ...
                <button
                  ...
                  style={{marginLeft:10}}
                  onClick={() =>
                    setFieldValue("drillExecutionDetails", [
                      ...values.drillExecutionDetails,
                      {
                        stepDescription:"",
                        responsible:"",
                        startTime:"",
            ...
            <FieldArray name="drillExecutionDetails">
            {({remove})=>(
              <table className="items-table">
                ...
                <tbody>
                  {values.drillExecutionDetails.map((row,index)=>(
                    <tr key={index}>
                      <td><Field as="textarea" name={`drillExecutionDetails.${index}.stepDescription`} className="form-input"/></td>
                      <td><Field name={`drillExecutionDetails.${index}.responsible`} className="form-input"/></td>
                      <td><Field type="time" name={`drillExecutionDetails.${index}.startTime`} className="form-input"/></td>
                      <td><Field type="time" name={`drillExecutionDetails.${index}.endTime`} className="form-input"/></td>
                      <td><Field as="textarea" name={`drillExecutionDetails.${index}.outcome`} className="form-input"/></td>
                      <td><Field as="textarea" name={`drillExecutionDetails.${index}.remarks`} className="form-input"/></td>

                      {dynamicColumns.map(col=>(
                        <td key={col.key}>
                          <Field name={`drillExecutionDetails.${index}.dynamicFields.${col.key}`} className="form-input"/>
                        </td>
                      ))}
          ...
          {/* 6. Results & Findings */}
          <div className="form-section">
            <h3 className="form-section-title">Results & Findings</h3>
            <div className="form-fields">
              {[
          ...
          {!isPrintMode &&
            <div className="form-actions">
              <button type="submit" className="btn-submit">
                Submit Traceability Drill Report
              </button>
            </div>
          }
...
export default FRM01487_TraceabilityDrillReport;