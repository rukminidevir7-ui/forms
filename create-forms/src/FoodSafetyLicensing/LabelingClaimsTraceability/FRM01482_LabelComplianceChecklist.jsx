// FRM01482_LabelComplianceChecklist.jsx
// FRM-01482 – Label Compliance Checklist
...
const initialValues = {
  ...
  /* 4. Label Compliance Checklist Items */
  labelChecklistItems: [
    {
...
const FRM01482_LabelComplianceChecklist = () => {
  ...
  return(
    <ModernFormWrapper formId="FRM-01482" title="Label Compliance Checklist">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Label Compliance Checklist Submitted Successfully");
        }}
      >
      {({values,setFieldValue})=>(
        <Form>
          <ModernA4Template
            formId="FRM-01482"
            title="FRM-01482 — Label Compliance Checklist"
          ...
          <div className="form-section">
            <h3 className="form-section-title">Label Compliance Checklist Items</h3>
            ...
            {!isPrintMode &&
              <div style={{marginBottom:15}}>
                ...
                <button
                  ...
                  style={{marginLeft:10}}
                  onClick={() =>
                    setFieldValue("labelChecklistItems", [
                      ...values.labelChecklistItems,
                      {
                        complianceRequirement:"",
                        referenceRegulation:"",
                        regulationCompliance:"",
            ...
            <FieldArray name="labelChecklistItems">
            {({remove})=>(
              <table className="items-table">
                ...
                <tbody>
                  {values.labelChecklistItems.map((row,index)=>(
                    <tr key={index}>
                      <td><Field name={`labelChecklistItems.${index}.complianceRequirement`} className="form-input"/></td>
                      <td><Field name={`labelChecklistItems.${index}.referenceRegulation`} className="form-input"/></td>

                      {/* Dropdown Yes/No */}
                      <td>
                        <Field
                          as="select"
                          name={`labelChecklistItems.${index}.regulationCompliance`}
                          className="form-input"
                        >
                      ...
                      <td><Field as="textarea" name={`labelChecklistItems.${index}.observation`} className="form-input"/></td>
                      <td><Field as="textarea" name={`labelChecklistItems.${index}.correctiveAction`} className="form-input"/></td>
                      <td><Field name={`labelChecklistItems.${index}.responsible`} className="form-input"/></td>
                      <td><Field type="date" name={`labelChecklistItems.${index}.targetDate`} className="form-input"/></td>
                      <td><Field as="textarea" name={`labelChecklistItems.${index}.remarks`} className="form-input"/></td>

                      {dynamicColumns.map(col=>(
                        <td key={col.key}>
                          <Field
                            name={`labelChecklistItems.${index}.dynamicFields.${col.key}`}
                            className="form-input"
                          />
                        </td>
                      ))}
          ...
          {!isPrintMode &&
            <div className="form-actions">
              <button type="submit" className="btn-submit">
                Submit Label Compliance Checklist
              </button>
            </div>
          }
...
export default FRM01482_LabelComplianceChecklist;