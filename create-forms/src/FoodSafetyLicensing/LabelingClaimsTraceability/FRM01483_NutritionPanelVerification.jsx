// FRM01483_NutritionPanelVerification.jsx
// FRM-01483 – Nutrition Panel Verification
...
const initialValues = {
  ...
  /* 4. Nutrition Panel Verification Items */
  nutritionVerificationItems: [
    {
      nutrientParameter: "",
      declaredValue: "",
      referenceStandard: "",
...
const FRM01483_NutritionPanelVerification = () => {
  ...
  return(
    <ModernFormWrapper formId="FRM-01483" title="Nutrition Panel Verification">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Nutrition Panel Verification Submitted Successfully");
        }}
      >
      {({values,setFieldValue})=>(
        <Form>
          <ModernA4Template
            formId="FRM-01483"
            title="FRM-01483 — Nutrition Panel Verification"
          ...
          <div className="form-section">
            <h3 className="form-section-title">Nutrition Panel Verification Items</h3>
            ...
            {!isPrintMode &&
              <div style={{marginBottom:15}}>
                ...
                <button
                  ...
                  style={{marginLeft:10}}
                  onClick={() =>
                    setFieldValue("nutritionVerificationItems", [
                      ...values.nutritionVerificationItems,
                      {
                        nutrientParameter:"",
                        declaredValue:"",
                        referenceStandard:"",
            ...
            <FieldArray name="nutritionVerificationItems">
            {({remove})=>(
              <table className="items-table">
                ...
                <tbody>
                  {values.nutritionVerificationItems.map((row,index)=>(
                    <tr key={index}>
                      <td><Field name={`nutritionVerificationItems.${index}.nutrientParameter`} className="form-input"/></td>
                      <td><Field name={`nutritionVerificationItems.${index}.declaredValue`} className="form-input"/></td>
                      <td><Field name={`nutritionVerificationItems.${index}.referenceStandard`} className="form-input"/></td>
                      <td><Field name={`nutritionVerificationItems.${index}.toleranceLimit`} className="form-input"/></td>

                      {/* Yes/No Dropdown */}
                      <td>
                        <Field
                          as="select"
                          name={`nutritionVerificationItems.${index}.compliance`}
                          className="form-input"
                        >
                      ...
                      <td><Field as="textarea" name={`nutritionVerificationItems.${index}.observation`} className="form-input"/></td>
                      <td><Field as="textarea" name={`nutritionVerificationItems.${index}.correctiveAction`} className="form-input"/></td>
                      <td><Field as="textarea" name={`nutritionVerificationItems.${index}.remarks`} className="form-input"/></td>

                      {dynamicColumns.map(col=>(
                        <td key={col.key}>
                          <Field
                            name={`nutritionVerificationItems.${index}.dynamicFields.${col.key}`}
                            className="form-input"
                          />
                        </td>
                      ))}
          ...
          {!isPrintMode &&
            <div className="form-actions">
              <button type="submit" className="btn-submit">
                Submit Nutrition Panel Verification
              </button>
            </div>
          }
...
export default FRM01483_NutritionPanelVerification;