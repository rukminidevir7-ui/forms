// FRM01485_BatchCodingLog.jsx
// FRM-01485 – Batch Coding Log
...
const FRM01485_BatchCodingLog = () => {
  ...
  return(
    <ModernFormWrapper formId="FRM-01485" title="Batch Coding Log">
      <Formik
        initialValues={initialValues}
        validationSchema={validationSchema}
        onSubmit={(values)=>{
          console.log(values);
          alert("Batch Coding Log Submitted Successfully");
        }}
      >
      {({values,setFieldValue})=>(
        <Form>
          <ModernA4Template
            formId="FRM-01485"
            title="FRM-01485 — Batch Coding Log"
          ...
          <div className="form-section">
            <h3 className="form-section-title">Batch Coding Entries</h3>
            ...
            {!isPrintMode &&
              <div style={{marginBottom:15}}>
                ...
                <button
                  ...
                  style={{marginLeft:10}}
                  onClick={() =>
                    setFieldValue("batchCodingEntries", [
                      ...values.batchCodingEntries,
                      {
                        batchNumber:"",
                        manufacturingDate:"",
                        expiryDate:"",
            ...
            <FieldArray name="batchCodingEntries">
            {({remove})=>(
              <table className="items-table">
                ...
                <tbody>
                  {values.batchCodingEntries.map((row,index)=>(
                    <tr key={index}>
                      <td><Field name={`batchCodingEntries.${index}.batchNumber`} className="form-input"/></td>
                      <td><Field type="date" name={`batchCodingEntries.${index}.manufacturingDate`} className="form-input"/></td>
                      <td><Field type="date" name={`batchCodingEntries.${index}.expiryDate`} className="form-input"/></td>
                      <td><Field name={`batchCodingEntries.${index}.codingMachine`} className="form-input"/></td>
                      <td><Field name={`batchCodingEntries.${index}.line`} className="form-input"/></td>
                      <td><Field name={`batchCodingEntries.${index}.operator`} className="form-input"/></td>
                      <td><Field name={`batchCodingEntries.${index}.verificationStatus`} className="form-input"/></td>
                      <td><Field as="textarea" name={`batchCodingEntries.${index}.remarks`} className="form-input"/></td>

                      {dynamicColumns.map(col=>(
                        <td key={col.key}>
                          <Field
                            name={`batchCodingEntries.${index}.dynamicFields.${col.key}`}
                            className="form-input"
                          />
                        </td>
                      ))}
...
export default FRM01485_BatchCodingLog;