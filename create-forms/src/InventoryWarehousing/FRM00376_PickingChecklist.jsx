// FRM00376_PickingChecklist.jsx
// FRM-00376 – Picking Checklist – Checklist Form
...
const validationSchema = Yup.object({
  ...
  pickingReference: Yup.string().required('Required'),
  ...
  materialItems: Yup.array().of(
    Yup.object().shape({
      ...
      quantityPicked: Yup.number().required('Required'),
...
const defaultVerificationChecks = [
  { item: 'Picking list verified material details', response: '', remarks: '' },
  { item: 'Correct storage location picked', response: '', remarks: '' },
  { item: 'Quantity verified', response: '', remarks: '' },
  { item: 'Batch / lot verified', response: '', remarks: '' },
  { item: 'No damage observed', response: '', remarks: '' },
  { item: 'FIFO / FEFO followed', response: '', remarks: '' },
  { item: 'Items properly labeled', response: '', remarks: '' },
  { item: 'System entry completed', response: '', remarks: '' },
  { item: 'Packing requirements followed', response: '', remarks: '' },
...
const FRM00376_PickingChecklist = () => {
  ...
  return (
    <ModernFormWrapper
      formId="FRM-00376"
      title="Picking Checklist – Checklist Form"
    >
      <Formik
        ...
        onSubmit={(values) => {
          console.log(values);
          alert('Picking Checklist submitted successfully');
        }}
      >
        {({ values }) => (
          <Form>
            ...
            >
              ...
              <div className="form-section">
                ...
                <div className="form-fields">
                  ...
                  {field(values,'pickingReference','Picking Reference / Order No.')}
                  {field(values,'checkedBy','Checked By')}
                  {field(values,'shiftTime','Shift / Time')}
                </div>
              </div>

              {/* 2. Order / Material Details with Dynamic Columns */}
        ...
        <tbody>
          {values.materialItems.map((item, index) => (
            <tr key={index}>
              <td>
                <Field
                  name={`materialItems.${index}.orderNo`}
              ...
              <td>
                <Field
                  name={`materialItems.${index}.quantityRequired`}
                  type="number"
                  className="form-input"
                />
              </td>
              ...
              <td>
                <Field
                  name={`materialItems.${index}.quantityPicked`}
                  type="number"
                  className="form-input"
                />
              </td>
              ...
              <div className="form-section">
                <h3 className="form-section-title">3. Picking Verification Checks</h3>
...
export default FRM00376_PickingChecklist;
